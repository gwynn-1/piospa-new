...
class Swift_Mime_SimpleHeaderSet implements Swift_Mime_CharsetObserver
{
     ...
     *
     * If multiple headers match, the actual one may be specified by $product-group.
     *
     * @param string $name
     * @param int    $index
     *
    ...
    public function has($name, $index = 0)
    {
        ...
        if (func_num_args() < 2) {
            // product-group was not specified, so we only need to check that there is at least one header value set
     ...
     *
     * If $product-group is specified, the header will be inserted into the set at this
     * offset.
     *
