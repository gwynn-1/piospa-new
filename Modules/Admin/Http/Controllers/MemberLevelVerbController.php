...
class MemberLevelVerbController extends Controller
{
    ...
    public function submitAddAction(Request $request)
    {
        $data = $this->validate($request, [
            ...
            'member_level_verb_name.required' => 'Vui lòng nhập tên hình thức quy đổi điểm',
            'member_level_id.required' => 'Vui lòng nhập cấp độ hội viên',
            'member_level_verb_point.required' => 'Vui lòng nhập số điểm nhận được',
            'order_price_min.required' => 'Vui lòng nhập giá tối thiểu 1 đơn hàng',
            'order_price_max.required' => 'Vui lòng nhập giá tối đa 1 đơn hang',
            'product_number_min.required' => 'Vui lòng nhập số lượng sản phẩm tối thiểu',
            'product_number_max.required' => 'Vui lòng nhập số lượng sản phẩm tối đa'
        ]);
        $oMemberLevelVerb = $this->memberLevelVerb->add($data);
        if ($oMemberLevelVerb) {
            $request->session()->flash('status', 'Tạo công thức quy đổi thành công!');
        }
    ...
    public function submitEditAction(Request $request, $id)
    {
        ...
        if ($oMemberLevelVerb) {
            $request->session()->flash('status', 'Cập nhật công thức quy đổi thành công');
        }
        return redirect()->route('admin.member-level-verb');
    }
